    ...
    try:
        with st.spinner("Fetching assets..."):
            ...
            response = requests.get(
                asset_info_url, headers={"Accept": "application/json", "x-api-key": api_key}, verify=False, timeout=timeout
            )
...
def getServerStatistics(immich_server_url, api_key):
    try:
        response = requests.get(
            f"{immich_server_url}/api/server-info/statistics", headers={"Accept": "application/json", "x-api-key": api_key}
        )
        if response.ok:
...
def getVideoAndSave(asset_id, immich_server_url, api_key, save_directory):
    ...
    response = requests.get(
        f"{immich_server_url}/api/download/asset/{asset_id}",
        headers={"Accept": "application/octet-stream", "x-api-key": api_key},
        stream=True,
    )
