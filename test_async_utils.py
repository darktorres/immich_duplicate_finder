...
class TestAsyncProcessor:
    ...
    async def test_process_batch_async_with_progress(self):
        ...
        items = [1, 2, 3, 4, 5]
        results = await processor.process_batch_async(items, identity, batch_size=2, progress_callback=progress_callback)
...
class TestRunWithProgress:
    ...
    def test_run_with_progress_with_callbacks(self):
        ...
        result = run_with_progress(test_func, progress_callback=progress_callback, error_callback=error_callback)
    ...
    def test_run_with_progress_with_exception(self):
        ...
        with pytest.raises(ValueError, match="Test error"):
            run_with_progress(failing_func, progress_callback=progress_callback, error_callback=error_callback)
...
async def test_async_processor_integration():
    ...
    results = await processor.process_batch_async(items, simulate_work, batch_size=3, progress_callback=track_progress)
